# FAL：Flash 抽象层
链接：https://github.com/RT-Thread-packages/fal

# 移植使用
中间库原名为fal，为避免与我们用户程序中的fal.c，fal.h重名，名字改为rtt_fal（用户程序中的fal_init和fal_deinit函数可能仍有冲突）

根据板卡和需求，在自己的工程添加自定义的rtt_fal_flash_xxx_port.c和rtt_fal_cfg.h进行编译，使用时调用fal_init即可
