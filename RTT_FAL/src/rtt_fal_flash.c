...
#error "You must defined flash device table (FAL_FLASH_DEV_TABLE) on 'rtt_fal_cfg.h'"
#endif
...
int fal_flash_init(void)
{
    ...
    for (i = 0; i < device_table_len; i++)
    {
        ...
        fal_flash_log_d("Flash device | %*.*s | addr: 0x%08lx | len: 0x%08x | blk_size: 0x%08x |initialized finish.",
