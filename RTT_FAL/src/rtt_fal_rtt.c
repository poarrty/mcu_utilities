...
struct rt_device *fal_blk_device_create(const char *parition_name)
{
    ...
    if (!fal_part)
    {
        fal_flash_log_e("Error: the partition name (%s) is not found.", parition_name);
        return NULL;
    }

    if ((fal_flash = fal_flash_device_find(fal_part->flash_name)) == NULL)
    {
        fal_flash_log_e("Error: the flash device name (%s) is not found.", fal_part->flash_name);
        return NULL;
    }
    ...
    if (blk_dev)
    {
        ...
        fal_flash_log_i("The FAL block device (%s) created successfully", fal_part->name);
        rt_device_register(RT_DEVICE(blk_dev), fal_part->name, RT_DEVICE_FLAG_RDWR | RT_DEVICE_FLAG_STANDALONE);
    }
    else
    {
        fal_flash_log_e("Error: no memory for create FAL block device");
    }
...
struct rt_device *fal_mtd_nor_device_create(const char *parition_name)
{
    ...
    if (mtd_nor_dev)
    {
        ...
        fal_flash_log_i("The FAL MTD NOR device (%s) created successfully", fal_part->name);
        rt_mtd_nor_register_device(fal_part->name, &mtd_nor_dev->parent);
    }
    else
    {
        fal_flash_log_e("Error: no memory for create FAL MTD NOR device");
    }
...
struct rt_device *fal_char_device_create(const char *parition_name)
{
    ...
    if (char_dev)
    {
        ...
        fal_flash_log_i("The FAL char device (%s) created successfully", fal_part->name);
    ...
    {
        fal_flash_log_e("Error: no memory for create FAL char device");
    }
