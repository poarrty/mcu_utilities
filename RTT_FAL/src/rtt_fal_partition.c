...
void fal_show_part_table(void)
{
    ...
    fal_flash_log_i("==================== FAL partition table ====================");
    fal_flash_log_i("| %-*.*s | %-*.*s |   offset   |    length  |", part_name_max, FAL_DEV_NAME_MAX, item1, flash_dev_name_max,
    ...
    for (i = 0; i < partition_table_len; i++)
    {
        ...
        fal_flash_log_i("| %-*.*s | %-*.*s | 0x%08lx | 0x%08x |", part_name_max, FAL_DEV_NAME_MAX, part->name, flash_dev_name_max,
...
int fal_partition_init(void)
{
    ...
    if (flash_dev == NULL)
    {
        fal_flash_log_e("Initialize failed! Flash device (%s) NOT found.", FAL_PART_TABLE_FLASH_DEV_NAME);
        goto _exit;
    }
    ...
    {
        fal_flash_log_e("Setting partition table end offset address(%ld) out of flash bound(<%d).", part_table_offset, flash_dev->len);
        goto _exit;
    }
    ...
    if (new_part == NULL)
    {
        fal_flash_log_e("Initialize failed! No memory for table buffer.");
        goto _exit;
    }
            ...
            if (flash_dev->ops.read(part_table_offset, read_buf, sizeof(read_buf)) > 0)
            {
                ...
                for (i = 0; i < sizeof(read_buf) - sizeof(read_magic_word) + 1; i++)
                {
                    ...
                    if (read_magic_word == ((FAL_PART_MAGIC_WORD_H << 16) + FAL_PART_MAGIC_WORD_L))
                    {
                        ...
                        part_table_offset += i;
                        fal_flash_log_d("Find the partition table on '%s' offset @0x%08lx.", FAL_PART_TABLE_FLASH_DEV_NAME,
                                part_table_offset);
                        break;
                    }
                }
    ...
    while (part_table_find_ok)
    {
        ...
        if (flash_dev->ops.read(part_table_offset - table_item_size * (table_num), (uint8_t *) new_part,
                table_item_size) < 0)
        {
            fal_flash_log_e("Initialize failed! Flash device (%s) read error!", flash_dev->name);
            table_num = 0;
            break;
        }
        ...
        {
            fal_flash_log_e("Initialize failed! No memory for partition table");
            table_num = 0;
            break;
        }
    ...
    if (table_num == 0)
    {
        fal_flash_log_e("Partition table NOT found on flash: %s (len: %d) from offset: 0x%08x.", FAL_PART_TABLE_FLASH_DEV_NAME,
    ...
    for (i = 0; i < partition_table_len; i++)
    {
        ...
        if (flash_dev == NULL)
        {
            fal_flash_log_d("Warning: Do NOT found the flash device(%s).", partition_table[i].flash_name);
            continue;
        }

        if (partition_table[i].offset >= (long)flash_dev->len)
        {
            fal_flash_log_e("Initialize failed! Partition(%s) offset address(%ld) out of flash bound(<%d).",
...
int fal_partition_read(const struct fal_partition *part, uint32_t addr, uint8_t *buf, size_t size)
{
    ...
    {
        fal_flash_log_e("Partition read error! Partition address out of bound.");
        return -1;
    }
    ...
    if (flash_dev == NULL)
    {
        fal_flash_log_e("Partition read error! Don't found flash device(%s) of the partition(%s).", part->flash_name, part->name);
        return -1;
    }
    ...
    if (ret < 0)
    {
        fal_flash_log_e("Partition read error! Flash device(%s) read error!", part->flash_name);
    }
...
int fal_partition_write(const struct fal_partition *part, uint32_t addr, const uint8_t *buf, size_t size)
{
    ...
    if (addr + size > part->len)
    {
        fal_flash_log_e("Partition write error! Partition address out of bound.");
        return -1;
    }
    ...
    if (flash_dev == NULL)
    {
        fal_flash_log_e("Partition write error!  Don't found flash device(%s) of the partition(%s).", part->flash_name, part->name);
        return -1;
    }
    ...
    if (ret < 0)
    {
        fal_flash_log_e("Partition write error! Flash device(%s) write error!", part->flash_name);
    }
...
int fal_partition_erase(const struct fal_partition *part, uint32_t addr, size_t size)
{
    ...
    if (addr + size > part->len)
    {
        fal_flash_log_e("Partition erase error! Partition address out of bound.");
        return -1;
    }
    ...
    if (flash_dev == NULL)
    {
        fal_flash_log_e("Partition erase error! Don't found flash device(%s) of the partition(%s).", part->flash_name, part->name);
        return -1;
    }
    ...
    if (ret < 0)
    {
        fal_flash_log_e("Partition erase error! Flash device(%s) erase error!", part->flash_name);
    }
